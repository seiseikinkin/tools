(function () {
    ...
    while (getPasteElement(index)) {
        ...
        if (!paste.includes("EVs: ")) {
            index++;
            continue;
        }

        const name = pasteElement.textContent.replaceAll(/\n.*/g, "").replaceAll(/@.*/g, "").trim();
        const item = paste.includes(" @ ") ? pasteElement.textContent.replaceAll(/.*@ /g, "").replaceAll(/\n.*/g, "").trim() : "none";
        const teraType = paste.includes("Tera Type: ") ? paste.replaceAll(/.*Tera Type: /g, "").replaceAll(/ .*/g, "") : "none";
        const nature = [paste.includes(" Nature  ") ? paste.replaceAll(/ Nature.*/g, "").replaceAll(/.* /g, "") : "none", 1, 1, 1, 1, 1];
        const base = [0, 0, 0, 0, 0, 0];
        const ivs = [31, 31, 31, 31, 31, 31];
        const evs = [0, 0, 0, 0, 0, 0];
        const stats = [0, 0, 0, 0, 0, 0];
        setNature(nature);
        setBase(pokedex, name, base);
        setIvs(paste, ivs);
        setEvs(paste, evs);
        setStats(base, ivs, evs, nature, stats);
...
function getPasteElement(index) {
    return document.querySelector("body > article:nth-child(" + index + ") > pre");
}

function getImageElement(index) {
    return document.querySelector("body > article:nth-child(" + index + ") > div");
}
...
function setNature(nature) {
    if (nature[0] === "Adamant") {
        nature[1] = 1.1;
        nature[3] = 0.9;
    } else if (nature[0] === "Bold") {
        nature[2] = 1.1;
        nature[1] = 0.9;
    } else if (nature[0] === "Brave") {
        nature[1] = 1.1;
        nature[5] = 0.9;
    } else if (nature[0] === "Calm") {
        nature[4] = 1.1;
        nature[1] = 0.9;
    } else if (nature[0] === "Careful") {
        nature[4] = 1.1;
        nature[3] = 0.9;
    } else if (nature[0] === "Gentle") {
        nature[4] = 1.1;
        nature[2] = 0.9;
    } else if (nature[0] === "Hasty") {
        nature[5] = 1.1;
        nature[2] = 0.9;
    } else if (nature[0] === "Impish") {
        nature[2] = 1.1;
        nature[3] = 0.9;
    } else if (nature[0] === "Jolly") {
        nature[5] = 1.1;
        nature[3] = 0.9;
    } else if (nature[0] === "Lax") {
        nature[2] = 1.1;
        nature[4] = 0.9;
    } else if (nature[0] === "Lonely") {
        nature[1] = 1.1;
        nature[2] = 0.9;
    } else if (nature[0] === "Mild") {
        nature[3] = 1.1;
        nature[2] = 0.9;
    } else if (nature[0] === "Modest") {
        nature[3] = 1.1;
        nature[1] = 0.9;
    } else if (nature[0] === "Naive") {
        nature[3] = 1.1;
        nature[4] = 0.9;
    } else if (nature[0] === "Naughty") {
        nature[1] = 1.1;
        nature[4] = 0.9;
    } else if (nature[0] === "Quiet") {
        nature[3] = 1.1;
        nature[5] = 0.9;
    } else if (nature[0] === "Rash") {
        nature[3] = 1.1;
        nature[4] = 0.9;
    } else if (nature[0] === "Relaxed") {
        nature[2] = 1.1;
        nature[5] = 0.9;
    } else if (nature[0] === "Sassy") {
        nature[4] = 1.1;
        nature[5] = 0.9;
    } else if (nature[0] === "Timid") {
        nature[5] = 1.1;
        nature[1] = 0.9;
    }
}

function setBase(pokedex, name, base) {
    if (pokedex[name] && pokedex[name]["bs"]) {
        base[0] = Number(pokedex[name]["bs"]["hp"]);
        base[1] = Number(pokedex[name]["bs"]["at"]);
        base[2] = Number(pokedex[name]["bs"]["df"]);
        base[3] = Number(pokedex[name]["bs"]["sa"]);
        base[4] = Number(pokedex[name]["bs"]["sd"]);
        base[5] = Number(pokedex[name]["bs"]["sp"]);
    }
}

function setIvs(paste, ivs) {
    if (paste.includes("IVs:")) {
        for (const iv of paste.replaceAll(/.*IVs: /g, "").replaceAll(/ {2}.*/g, "").split(" / ")) {
            if (iv.includes("HP")) {
                ivs[0] = Number(iv.replaceAll(/HP/g, "").trim());
            } else if (iv.includes("Atk")) {
                ivs[1] = Number(iv.replaceAll(/Atk/g, "").trim());
            } else if (iv.includes("Def")) {
                ivs[2] = Number(iv.replaceAll(/Def/g, "").trim());
            } else if (iv.includes("SpA")) {
                ivs[3] = Number(iv.replaceAll(/SpA/g, "").trim());
            } else if (iv.includes("SpD")) {
                ivs[4] = Number(iv.replaceAll(/SpD/g, "").trim());
            } else if (iv.includes("Spe")) {
                ivs[5] = Number(iv.replaceAll(/Spe/g, "").trim());
            }
        }
    }
}

function setEvs(paste, evs) {
    for (const ev of paste.replaceAll(/.*EVs: /g, "").replaceAll(/ {2}.*/g, "").split(" / ")) {
        if (ev.includes("HP")) {
            evs[0] = Number(ev.replaceAll(/HP/g, "").trim());
        } else if (ev.includes("Atk")) {
            evs[1] = Number(ev.replaceAll(/Atk/g, "").trim());
        } else if (ev.includes("Def")) {
            evs[2] = Number(ev.replaceAll(/Def/g, "").trim());
        } else if (ev.includes("SpA")) {
            evs[3] = Number(ev.replaceAll(/SpA/g, "").trim());
        } else if (ev.includes("SpD")) {
            evs[4] = Number(ev.replaceAll(/SpD/g, "").trim());
        } else if (ev.includes("Spe")) {
            evs[5] = Number(ev.replaceAll(/Spe/g, "").trim());
        }
    }
}

function setStats(base, ivs, evs, nature, stats) {
    stats[0] = base[0] + 60 + parseInt(ivs[0] / 2.0 + evs[0] / 8.0);
    stats[1] = parseInt((base[1] + 5 + parseInt(ivs[1] / 2.0 + evs[1] / 8.0)) * nature[1]);
    stats[2] = parseInt((base[2] + 5 + parseInt(ivs[2] / 2.0 + evs[2] / 8.0)) * nature[2]);
    stats[3] = parseInt((base[3] + 5 + parseInt(ivs[3] / 2.0 + evs[3] / 8.0)) * nature[3]);
    stats[4] = parseInt((base[4] + 5 + parseInt(ivs[4] / 2.0 + evs[4] / 8.0)) * nature[4]);
    stats[5] = parseInt((base[5] + 5 + parseInt(ivs[5] / 2.0 + evs[5] / 8.0)) * nature[5]);
}

function createInfoElement(stats, nature, item) {
    ...
    statsAtElement.innerHTML = stats[1];
    if (nature[1] === 1.1) {
    ...
    statsDfElement.innerHTML = stats[2];
    if (nature[2] === 1.1) {
    ...
    statsSaElement.innerHTML = stats[3];
    if (nature[3] === 1.1) {
    ...
    statsSdElement.innerHTML = stats[4];
    if (nature[4] === 1.1) {
    ...
    statsSpElement.innerHTML = stats[5];
    if (nature[5] === 1.1) {
    ...
    const newline1Element = document.createElement('span');
    newline1Element.innerHTML = "\n";
    infoElement.append(newline1Element);

    const durabilityLabelElement = document.createElement('span');
    durabilityLabelElement.innerHTML = "Durability: ";
    durabilityLabelElement.style.color = "#a0a0a0";
    infoElement.append(durabilityLabelElement);
    ...
    if (item === "Eviolite") {
        durabilityHbElement.innerHTML = parseInt(stats[0] * stats[2] * 1.5) + "*";
    } else {
    ...
    if (item === "Eviolite" || item === "Assault Vest") {
        durabilityHdElement.innerHTML = parseInt(stats[0] * stats[4] * 1.5) + "*";
    } else {
    ...
    //infoElement.style.width = "380px";
    infoElement.style.marginTop = "-16px";
    infoElement.style.marginBottom = "12px";
    infoElement.style.paddingTop = "4px";
    infoElement.style.paddingBottom = "4px";
    infoElement.style.paddingLeft = "12px";
    infoElement.style.paddingRight = "12px";
    infoElement.style.borderRadius = "8px";

    return infoElement;
}

function getTeraTypeIcon(type) {
    const teraTypeIconUrl = 'https://seiseikinkin.github.io/tools/image/teratypeicon/' + type.toLowerCase() + '.png';

    // const teraTypeIconElement = document.createElement('img');
    // teraTypeIconElement.classList.add("img-tera");
    // teraTypeIconElement.src = teraTypeIconUrl;
    // return teraTypeIconElement;

    const teraTypeIconElement = document.createElement('div');
    teraTypeIconElement.style.display = 'inline-block';
    teraTypeIconElement.style.backgroundImage = 'url(' + teraTypeIconUrl + ')';
    teraTypeIconElement.style.backgroundRepeat = 'no-repeat';
    teraTypeIconElement.style.backgroundPosition = 'center';
    teraTypeIconElement.style.backgroundSize = 'contain';
    teraTypeIconElement.style.marginLeft = '-4px';
    teraTypeIconElement.style.marginBottom = '4px';
    teraTypeIconElement.style.width = '26px';
    teraTypeIconElement.style.height = '26px';
    return teraTypeIconElement;
}
    ...
    sources.forEach(source => {
        for (let key in source) {
            if (source[key] && typeof source[key] === 'object' && !Array.isArray(source[key])) {
                // オブジェクトがネストされている場合、再帰的にコピー
                if (!target[key]) target[key] = {};
                deepExtend(target[key], source[key]);
            } else {
                // プリミティブ値または配列はそのままコピー
...
function getPokedex() {
    ...
    var POKEDEX_GSC = deepExtend({}, POKEDEX_RBY, {
        //TYPE CHANGES
        "Magnemite": { "t2": "Steel" },
        "Magneton": { "t2": "Steel" },
        //GS Pokemon
    ...
    var POKEDEX_ADV = deepExtend({}, POKEDEX_GSC, {
        //RS Pokemon
        "Absol": {
            "t1": "Dark",
            "bs": {
    ...
    var POKEDEX_DPP = deepExtend({}, POKEDEX_ADV, {
        //ABILITY CHANGES
    ...
    var POKEDEX_BW = deepExtend({}, POKEDEX_DPP, {
        //Type Changes
        "Rotom-Mow": { "t2": "Grass" },
        "Rotom-Frost": { "t2": "Ice" },
        "Rotom-Heat": { "t2": "Fire" },
    ...
    var POKEDEX_XY = deepExtend({}, POKEDEX_BW, {
        //new forms
    ...
    var POKEDEX_SM = deepExtend({}, POKEDEX_XY, {
        //new forms
    ...
    var POKEDEX_SS_NATDEX = deepExtend({}, POKEDEX_SM, {
        //SWSH Pokemon
        "Meltan": {
            "t1": "Steel",
            "bs": {
    ...
    var POKEDEX_SS = deepExtend({}, POKEDEX_SS_NATDEX, {
    ...
    var POKEDEX_LA = deepExtend({}, POKEDEX_SS_NATDEX, {
        "Growlithe-Hisui": {
            "t1": "Fire",
            "t2": "Rock",
            "bs": {
    ...
    var POKEDEX_SV_NATDEX = deepExtend({}, POKEDEX_LA, {
    ...
    POKEDEX_SV = deepExtend({}, POKEDEX_SV);
