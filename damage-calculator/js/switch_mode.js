...
$(function () {
    ...
    $('#gen7sets').on('click', function () {
        ...
        loadSetdexScript();
        if (!$('#p1' + ' .set-toggle').prop('checked')) loadPreset('#p1', tempSetName);
        if (!$('#p2' + ' .set-toggle').prop('checked')) loadPreset('#p2', tempSetName);
    });
...
function loadDex(dexMode) {
    ...
    if (typeChart !== undefined) {
        ...
        if (types.indexOf('Typeless') !== -1) types.splice(types.indexOf('Typeless'), 1);
        var teraTypes = $.extend(true, [], types);
        if (gen >= 2) types.push('Typeless');
        if (types.indexOf('Stellar') !== -1) types.splice(types.indexOf('Stellar'), 1);
