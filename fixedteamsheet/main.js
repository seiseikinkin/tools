    ...
    var mo = new MutationObserver(function() {
        ...
        for (const room of document.getElementsByClassName('ps-room-opaque')) {
            if (executedRoomList.indexOf(room.id) < 0) {
                const leftTeamSheetButton = createFixedTeamSheetButton(room.id, true);
                document.querySelector('#' + room.id + ' > div.battle-log > div.battle-options > div').append(leftTeamSheetButton);
                const rightTeamSheetButton = createFixedTeamSheetButton(room.id, false);
...
function createFixedTeamSheetButton(id, isLeft) {
    ...
    if (isLeft) {
        fixedTeamSheetButtonElement.innerText= 'Left Team';
    } else {
        fixedTeamSheetButtonElement.innerText= 'Right Team';
    }
